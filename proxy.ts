...
import { URL_DASHBOARD_PAGE, URL_FILTERS_PAGE, URL_FORBIDDEN_PAGE, URL_LOGIN_PAGE, URL_REGISTER_PAGE } from './app/_utils/utils';
...
export async function proxy(request: NextRequest) {
    ...
    if (!cookie) {
        if (request.nextUrl.pathname.startsWith(URL_DASHBOARD_PAGE)) {
            return redirect(request, URL_FORBIDDEN_PAGE);     // Unauthenticated user is not allowed to navigate to account
        }
    }
    ...
    if (cookie) {
        ...
        if (session) {      // User is authenticated
            ...
            if (request.nextUrl.pathname.startsWith(URL_FILTERS_PAGE)) {
