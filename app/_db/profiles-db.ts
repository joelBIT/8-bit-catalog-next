 ...
 */
export async function updateProfileImageById(id: string, image: File): Promise<void> {
    await uploadFile(image, PROFILE_IMAGES_STORAGE, id + "/");   // uploads the image file to a bucket.
...
async function updateUserImage(id: string, image: string): Promise<void> {
